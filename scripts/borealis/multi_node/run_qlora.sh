...
mkdir -p "${output_dir}/${WW}/${model_config}/multi_node/qlora_logs"
...
mpiexec --envall -n ${WORLD_SIZE} -ppn ${GPUS_PER_NODE} \
  bash -lc '
    ...
    NODELOG="'"${output_dir}"'/'"${WW}"'/'"${model_config}"'/multi_node/qlora_logs/run_$(hostname -s)_r${RANK}.log"
    ...
    $PYTHON -m trl.scripts.sft --fsdp "full_shard auto_wrap" \
        ...
        --lora_alpha 16 \
        --use_peft \
        --output_dir "'"${output_dir}"'/'"${WW}"'/'"${model_config}"'/multi_node/qlora_logs" \
    |& tee -a "${NODELOG}"
  '
